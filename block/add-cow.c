...
static int add_cow_create(const char *filename, QemuOpts *opts)
{
    ...
    assert(strlen(backing_fmt ? backing_fmt : "") < sizeof(header.backing_fmt));
    strncpy(header.backing_fmt, backing_fmt ? backing_fmt : "",
            sizeof(header.backing_fmt));
    assert(strlen(image_format ? image_format : "raw") < sizeof(header.image_fmt));
                               ...
                               int remaining_sectors)
{
    ...
    QLIST_FOREACH(old_alloc, &s->cluster_allocs, next_in_flight) {
        ...
        uint64_t old_end = old_alloc->start_sector + old_alloc->remaining_sectors;
